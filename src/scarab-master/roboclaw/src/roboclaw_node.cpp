...
public:
  explicit DifferentialDriver(const ros::NodeHandle &node) :
    ...
    nh_.param("motor_to_wheel_ratio", motor_to_wheel_ratio_, 100.0); // 33.0
    ...
    nh_.param("pid_p", pid_p_, 1.9);
    nh_.param("pid_i", pid_i_, 3.0);
    nh_.param("pid_d", pid_d_, 0.1);
    nh_.param("pid_iclamp", pid_iclamp_, 2000.0);
    ...
    // (100 * 102 * 64) / 60
    duty_per_qpps_ = 500000; //(100 * 102 * 64) / 60;
  }
...
public:
  RoboClawNode() : priv_node_("~") {
    ...
    priv_node_.param("freq", freq_, 1.0); //default: 30 
